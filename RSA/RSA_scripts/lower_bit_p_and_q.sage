...
def main():
	...
	p0 = p & (2^(round(length_N/2)-hidden+1)-1)
	q0 = q & (2^(round(length_N/2)-hidden+1)-1)
	print("N: ",N)
	print("\np:  ",bin(p))
	print("p0: ",bin(p0))
	print("\nq:  ",bin(q))
	...
	f = (QQ(Z(p0/(2^(round(length_N/2)-hidden+1))))+x)
	f *= (QQ(Z(q0/(2^(round(length_N/2)-hidden+1))))+y)
	# рассчитаем корни, которые, по идее, должна найти наша программа (обычно мы их не знаем)
	x0 = (p - p0)/(2^(round(length_N/2)-hidden+1))
	y0 = (q - q0)/(2^(round(length_N/2)-hidden+1))
	X = 2^hidden
	Y = 2^hidden
	...
	print("\nожидаем x0:  ",x0)
	my_x = (N/(my_y*(2^(round(length_N/2)-hidden+1))+q0)-p0)/(2^(round(length_N/2)-hidden+1))
	print("получили x0: ", my_x)
