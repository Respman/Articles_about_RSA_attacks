...
def main():
	...
	N = p*q
	hidden = 32
	qbar = q & (2**(round(length_N/2)-hidden+1)-1)
	print("двоичное представление числа q:         ",bin(q))
	print("двоичное представление известных бит q: ",bin(qbar))
	G = Zmod(N)
	F.<x> = PolynomialRing(G, implementation='NTL')
	pol = G(qbar/(2**(round(length_N/2)-hidden+1))) + x
	...
	if (len(roots) > 0):
		print ("\n# Решение")
		print ("мы хотим увидеть:", (q - qbar)/(2**(round(length_N/2)-hidden+1)))
